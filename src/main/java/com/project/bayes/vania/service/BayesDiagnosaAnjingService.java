...
//		System.out.println("kesimpulan: " + resultPrediction.getName());
		...
		returnMap.put("dataTestingAnjing", newData);
...
//		probResults.entrySet().stream().forEach(p -> {
			...
			for (String result : results) {
				...
				for (String attrValue : attr.getValues()) {
					long countAttr = dataBayesList.stream().filter(p -> {
						...
						case "mengigitGigit":
							return p.getResult().equalsIgnoreCase(result) && p.getMengigitGigit().equalsIgnoreCase(attrValue);
						case "menjilatKaki":
							return p.getResult().equalsIgnoreCase(result) && p.getMenjilatKaki().equalsIgnoreCase(attrValue);
						case "buluRontok":
							return p.getResult().equalsIgnoreCase(result) && p.getBuluRontok().equalsIgnoreCase(attrValue);
						case "nafsuMakan":
							return p.getResult().equalsIgnoreCase(result) && p.getNafsuMakan().equalsIgnoreCase(attrValue);
						case "jamuran":
...
//								System.out.println("checkZero: " + checkZero);
								if (checkZero) {
									...
									mapAttr.setNewProb(newProb);

//									System.out.println("new mapAttr: " + mapAttr.toString());
...
//		mapAttrs.stream().filter(p->p.isSelected()).forEach(p -> {
//			System.out.println(p.toString());
//		});
